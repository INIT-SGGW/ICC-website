...
const initialData = {
    ...
    partA: new File([""], "partA.md"),
    partB: new File([""], "partB.md"),
}

export function UpdateTaskForm({ id }: Props) {
    const [title, setTitle] = useState<string>(initialData?.title || "");
    const [releaseDate, setReleaseDate] = useState<Date>(initialData?.releaseDate || new Date());
    const [partA, setPartA] = useState<File | undefined>(initialData?.partA || undefined);
    const [partB, setPartB] = useState<File | undefined>(initialData?.partB || undefined);
    const [error, setError] = useState<string | undefined>(undefined);

    useEffect(() => {
        // Adds current files to file inputs
        const hydrateFileInput = (file: File, inputID: string) => {
        ...
        if (partA) {
            hydrateFileInput(partA, "#part-a");
        }
        if (partB) {
            hydrateFileInput(partB, "#part-b");
        }
    }, [])
